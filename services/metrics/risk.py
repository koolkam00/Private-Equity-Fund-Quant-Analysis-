...
def compute_loss_and_distribution(deals, metrics_by_id=None):
    ...
    for d in deals:
        if d.equity_invested and d.equity_invested > 0:
    ...
    if deals_with_moic:
        ...
        total_eq = sum(d.equity_invested for d, _ in deals_with_moic)
        loss_eq = sum(d.equity_invested for d, _ in losses)
        loss_ratio_count = len(losses) / len(deals_with_moic) * 100
        loss_ratio_capital = safe_divide(loss_eq, total_eq)
        if loss_ratio_capital is not None:
            loss_ratio_capital *= 100
    else:
        losses = []
        loss_ratio_count = None
        loss_ratio_capital = None
